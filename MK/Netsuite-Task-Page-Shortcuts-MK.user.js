...
// @grant        GM_addStyle
// @run-at       docment-idle
// @updateURL    https://raw.githubusercontent.com/jmak-bw/MK/Netsuite-Task-Page-Shortcuts-MK.user.js
// @downloadURL  https://raw.githubusercontent.com/jmak-bw/MK/Netsuite-Task-Page-Shortcuts-MK.user.js
// ==/UserScript==
...
(function() {
    ...
    function extractSOID() {
        ...
        if (transactionDisplayElement) {
            ...
            else if (SOID.includes("Sales Order #SO-BWUK-2025-")) {
                const match = SOID.match(/Sales Order #SO-BWUK-2025-(\d{5})/);
                if (match) {
                    SOID = "SO-" + match[1];  // Replace with "SO-" + last 5 digits
                    console.log('extractSOID: Complete. Updated SOID for Sales Order:', SOID);
                } else {
                    console.log('extractSOID: Failed. No matching number found after "Sales Order #SO-BWUK-2025-"');
                }
            }
            // Check if SOID contains "Sales Order #SO<4-digit number>"
                ...
                if (match) {
                    ...
                    console.log('extractSOID: Failed. No matching number found after "Sales Order #SO"');
                        ...
                        // Check if SOID contains "Invoice #"
            else if (SOID.includes("Invoice")) {
                const match = SOID.match(/Invoice #INV-BWUK-2025-(\d{5})/);
                if (match) {
                    SOID = "INV #" + match[1];  // Update with "INV #" + number
                    console.log('extractSOID: Complete. Updated SOID for Invoice:', SOID);
                } else {
                    console.log('extractSOID: Failed. No number found after "Invoice #INV-BWUK-2025-"');
                }
            }

            // New condition: Check if SOID contains "Purchase Order #PO-BWUK-2025-"
            else if (SOID.includes("Purchase Order #PO-BWUK-2025-")) {
                const match = SOID.match(/Purchase Order #PO-BWUK-2025-(\d{5})/);
                if (match) {
                    SOID = "PO-" + match[1];  // Replace with "PO-" + last 5 digits
                    console.log('extractSOID: Complete. Updated SOID for Purchase Order:', SOID);
                } else {
                    console.log('extractSOID: Failed. No matching number found after "Purchase Order #PO-BWUK-2025-"');
                }
            }
            ...
            console.log('extractSOID: (Error) inpt_transaction_9 element not found.');
        }
    }
    ...
    if (window.location.href.includes('/app/crm/calendar/task.nl?l=T&transaction=')) {
